...
<div class="container">
  ...
  <h3 class="header smaller lighter blue">
    ...
    <p>
        <?= Html::a('Registrar Marca', ['create'], ['class' => 'btn btn-success']) ?>
    </p>
                            ...
                            'buttons' => [
                              'update' => function ($url, $model, $key) {
                                  return Html::a('<span class="btn btn-xs btn-primary"><i class="ace-icon fa fa-refresh bigger-120"></i></span> ',
                                      Url::to(['fact-marcas/update','id'=>$model->id]), [
                ...
                [
                      ...
                      'buttons' => [
                        'delete' => function ($url,$model, $key) {
                              $url=Url::to(['fact-marcas/delete','id'=>$model->id]);
                              return Html::a('<span class="btn btn-xs btn-danger"><i class="ace-icon fa fa-trash bigger-120"></i></span> ', '#', [
                                  'title' => Yii::t('yii', 'Delete'),
                                  'aria-label' => Yii::t('yii', 'Delete'),
                                  'onclick' => "
                                  krajeeDialog.confirm('Esta seguro de eliminar la Marca:  ' +  '$model->denominacion', function (result) {
                                       if (result) {
                                          $.ajax({
                                          ...
                                          success: function (json){
                                            $.pjax.reload({container: '#grid-marcas'});
